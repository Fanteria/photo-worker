...
void Convertor::reset_buffers() noexcept {
  ...
  for_each(
      compImageSizes.begin(), compImageSizes.end(), [](auto &size) { size = 0; });
}
...
void Convertor::save_jpg(
  ...
  if (*size < mem_image->data_size) {
    if (compressedImage != nullptr) tjFree(compressedImage);
    *size = 0;
  }

  // Compress and save image if it is bitmap
  tjCompress2(
      tjCompressors[procNum],
      mem_image->data,
      mem_image->width,
      0,
      mem_image->height,
  ...
  f.write(
      reinterpret_cast<char *>(compressedImage),
      static_cast<std::streamsize>(*size));
  f.close();
}

void Convertor::process_picture(
    const string &file_name,
    size_t procNum,
    shared_ptr<Pictures> pictures,
  ...
  if (convert) convert_picture(dest / (name + ".jpg"), procNum);
}

void Convertor::process_list(
    size_t procNum,
    atomic<size_t> *index,
...
string Convertor::get_info_string(
    size_t &last,
    size_t act,
    size_t max,
    const vector<string> &pics,
  ...
  info += "[";
  for (size_t i = 0; i < barLen; ++i) info += (i > actLen) ? "." : "#";
  info += "]";
...
void Convertor::print_info(
    const atomic<size_t> *index,
    const vector<string> *pics,
    size_t threadNum,
...
Convertor::Convertor(
    const fs::path &src_, const fs::path &dest_, size_t threads) noexcept
...
Convertor::~Convertor() noexcept {
  ...
  for_each(par_unseq, iProcessors.begin(), iProcessors.end(), [](auto &proc) {
    delete proc;
  });
  for_each(par_unseq, tjCompressors.begin(), tjCompressors.end(), [](auto &comp) {
    tjDestroy(comp);
  });
}
...
shared_ptr<Pictures> Convertor::conver_photos_list(
    const vector<string> &pics) noexcept {
  ...
  thread tq;
  if (!quiet) {
    tq = thread(Convertor::print_info, &index, &pics, iProcessors.size(), verbose);
  }
