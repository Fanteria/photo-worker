...
inline fs::path Worker::new_path(
    const fs::path &path, const string &name) noexcept {
  return path.parent_path() / name;
}

inline set<fs::path> Worker::get_sorted_files(
    const fs::path &path, const string *suffix) noexcept {
  set<fs::path> sorted_files;
  for (const auto &file : fs::directory_iterator(path)) {
    if (suffix && file.path().extension() != *suffix) continue;
    sorted_files.insert(file.path());
  }
  return sorted_files;
}

void Worker::remove_surplus_files(
    const fs::path &original, const fs::path &synced) noexcept {
...
Worker::Worker(
    const std::filesystem::path &src_, const std::filesystem::path &dest_) noexcept
    : src(src_), dest(dest_) {}
...
void Worker::read_files(
    vector<string> &files, const string &suffix, const fs::path &path) noexcept {
...
void Worker::rename_files_in_folder(
    const fs::path &path,
    const string &name,
    const string &suffix,
